...
use crate::triplestore::parquet::{property_to_filename, read_parquet, split_write_tmp_df, write_parquet};
use log::debug;
use oxrdf::vocab::xsd;
...
impl Triplestore {
            ...
            for (_, v) in map {
                if !v.unique {
                    if self.caching_folder.is_some() {
                        ...
                        let paths = split_write_tmp_df(self.caching_folder.as_ref().unwrap(), unique_df, predicate)?;
    ...
    fn add_triples_df_with_caching_folder(&mut self, mut triples_df: Vec<TripleDF>, call_uuid: &String) -> Result<(), MappingError>{
