...
pub(crate) fn delete_tmp_parquets_in_caching_folder(caching_folder: &Path) -> Result<(), TriplestoreError> {
    let contents = read_dir(caching_folder).map_err(|x|TriplestoreError::ReadCachingDirectoryError(x))?;
    for f in contents {
